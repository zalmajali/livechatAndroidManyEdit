...
@Component({
  selector: 'app-chatsarchivenew',
  templateUrl: './chatsarchivenew.page.html',
  styleUrls: ['./chatsarchivenew.page.scss'],
  standalone:false
})
export class ChatsarchivenewPage implements OnInit {
...
public dbInstance!: SQLiteObject;
  ...
  async ngOnInit() {
    ...
    await this.activaterouter.params.subscribe(async (params:any) => {
      ...
      if(params['name']!="" && params['name']!=null && params['name']!=undefined && params['name']!=0){
        this.selectNumberName = params['name'];
        await this.storage.set('selectedNumberName',this.selectNumberName);
      }
  ...
  functionBackHome(){
    this.navCtrl.navigateRoot(['/archive', {number:this.selectNumber,name:this.selectNumberName,chatSessionId:"",type:2}]);
  }
